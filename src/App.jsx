...
import { 
  Camera, RefreshCw, Upload, CheckCircle, AlertCircle, Download, 
  User, Trash2, Scissors, Zap, Sparkles, Palette, History, 
  Maximize2, Layout, Scan, Target, Focus, Grid, CameraIcon, 
  ArrowRightLeft, Image as ImageIcon, Sliders, Layers, MessageSquare, 
  Award, Type, Settings, ChevronRight
} from 'lucide-react';
...
const App = () => {
  ...
  // Chat State
  const [chatLog, setChatLog] = useState([{ role: 'ai', text: "Hello! I'm your AI Stylist. I've been optimized to protect your identity while recommending styles that suit your facial structure. How can I help you?" }]);
  ...
  const analyzeFace = async (base64) => {
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${ANALYSIS_MODEL}:generateContent?key=${API_KEY}`, {
        method: 'POST',
      ...
      setChatLog(prev => [...prev, { role: 'ai', text: `Scan complete! You have a ${analysis.shape} face shape. I recommend styles like: ${analysis.recommendedStyles.join(', ')} to balance your features.` }]);
  ...
  const generateTransformation = async () => {
    ...
    let lengthDesc = hairLength < 25 ? 'very short pixie' : hairLength < 50 ? 'chin-length bob' : hairLength < 75 ? 'shoulder length' : 'waist-length extra long';

    const transformationPrompt = `
      STRICT IDENTITY LOCK: You are a professional hair stylist. Keep the face, eyes, nose, lips, and skin texture 100% IDENTICAL to the source photo. DO NOT MODIFY FACIAL FEATURES OR PROPORTIONS.
      HAIR DESIGN TASK:
      - STYLE: ${selectedStyle.id === 'custom-made' ? `Bespoke design optimized for a ${faceAnalysis?.shape || 'balanced'} face` : selectedStyle.name}.
      - LENGTH: ${lengthDesc}.
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${IMAGE_EDIT_MODEL}:generateContent?key=${API_KEY}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (b64) {
        ...
        setResultImage(img);
        setHistory(prev => [{ id: Date.now(), image: img, style: selectedStyle.name }, ...prev].slice(0, 10));
      } else { throw new Error("Render limit reached. Try different parameters."); }
  ...
  const downloadCollage = () => {
    ...
    img1.onload = () => {
      img2.onload = () => {
        ...
        ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(20, 740, 120, 40); ctx.fillRect(620, 740, 120, 40);
        ctx.fillStyle = "white"; ctx.font = "bold 20px Arial";
        ctx.fillText("SOURCE", 40, 768); ctx.fillText("RENDER", 640, 768);
    ...
    <div className="min-h-screen bg-[#fcfdfe] text-slate-900 font-sans antialiased">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 px-6 h-16 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-bold text-xl tracking-tight">StyleLab<span className="text-indigo-600">.ai</span></span>
        ...
        <div className="flex gap-2 bg-slate-100 p-1 rounded-full">
          {['design', 'stylist', 'history'].map(tab => (
            <button 
              key={tab} 
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <section className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden group">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={startCamera} className="p-1.5 text-slate-300 hover:text-indigo-600 transition-colors"><CameraIcon className="w-4 h-4" /></button>
                {userPhoto && <button onClick={() => setShowMesh(!showMesh)} className={`p-1.5 rounded-lg transition-colors ${showMesh ? 'bg-indigo-100 text-indigo-600' : 'text-slate-300'}`}><Focus className="w-4 h-4" /></button>}
              </div>
            </div>
            ...
            {cameraActive ? (
              <div className="relative rounded-2xl overflow-hidden aspect-square bg-black shadow-inner">
                ...
                <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-4">
                  <button onClick={capturePhoto} className="p-4 bg-indigo-600 rounded-full text-white shadow-xl hover:scale-105 active:scale-95"><CameraIcon className="w-6 h-6" /></button>
                  <button onClick={stopCamera} className="p-4 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/30"><Trash2 className="w-6 h-6" /></button>
                </div>
              </div>
            ) : !userPhoto ? (
              <div onClick={() => fileInputRef.current.click()} className="border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/10 transition-all">
              ...
              <div className="space-y-4 animate-in fade-in duration-300">
                <div className="relative rounded-2xl overflow-hidden aspect-square border-4 border-white shadow-md">
                  ...
                  {analyzing && <div className="absolute inset-0 bg-indigo-900/30 backdrop-blur-sm flex items-center justify-center text-white text-[10px] font-black uppercase tracking-[0.3em] animate-pulse">Syncing Anatomy...</div>}
                </div>
                {faceAnalysis && (
              ...
              <section className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                    {BASE_STYLES.map(s => (
                      <button key={s.id} onClick={() => setSelectedStyle(s)} className={`p-3 text-left rounded-xl border-2 transition-all ${selectedStyle.id === s.id ? 'border-indigo-600 bg-indigo-50 shadow-sm' : 'border-slate-50 bg-slate-50 hover:border-slate-200'}`}>
                ...
                <div className="pt-6 border-t border-slate-100 space-y-6">
                  <div>
                    ...
                    <input type="range" value={hairLength} onChange={(e) => setHairLength(e.target.value)} className="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {TEXTURES.map(t => (
                        <button key={t.id} onClick={() => setSelectedTexture(t)} className={`py-2 rounded-xl border-2 text-[9px] font-black transition-all ${selectedTexture.id === t.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-slate-50 text-slate-400 border-slate-50'}`}>{t.id}</button>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-slate-100">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input type="checkbox" checked={highlights} onChange={() => setHighlights(!highlights)} className="w-4 h-4 accent-indigo-600" />
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    {HAIR_COLORS.map(c => (
                      <button 
                        ...
                        title={c.name}
                      >
                        {selectedColor.id === c.id && <div className="absolute inset-0 flex items-center justify-center"><CheckCircle className="w-4 h-4 text-white" /></div>}
                      </button>
                    ))}
                  ...
                  {highlights && (
                    <div className="space-y-2 animate-in slide-in-from-top-2">
                      <div className="flex justify-between text-[9px] font-black text-slate-400 uppercase"><span>Subtle</span><span>Highlight Intensity</span><span>Bold</span></div>
                      <input type="range" value={highlightIntensity} onChange={(e) => setHighlightIntensity(e.target.value)} className="w-full h-1 bg-indigo-100 rounded-lg appearance-none cursor-pointer accent-indigo-400" />
                    </div>
                  )}
                </div>
              </section>

              <button onClick={generateTransformation} disabled={loading || !userPhoto} className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white rounded-3xl font-black text-xl shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-3 active:scale-95 group">
                {loading ? <RefreshCw className="w-7 h-7 animate-spin" /> : <><Zap className="w-6 h-6 fill-white group-hover:scale-125 transition-transform" /> Render Look</>}
              </button>
            </div>
          ) : activeTab === 'stylist' ? (
            <section className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm h-[600px] flex flex-col animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="relative mt-auto pt-4 border-t border-slate-100">
                <input 
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && setChatLog([...chatLog, {role: 'user', text: chatInput}], setChatInput(""))}
                ...
                />
                <button className="absolute right-2 top-6 p-1.5 bg-indigo-600 rounded-xl text-white shadow-md"><ChevronRight className="w-4 h-4" /></button>
              </div>
            </section>
          ) : (
            <section className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm h-[600px] overflow-y-auto custom-scrollbar animate-in slide-in-from-right-4 duration-300">
              ...
              {history.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  {history.map(h => (
                    <button key={h.id} onClick={() => setResultImage(h.image)} className="rounded-2xl overflow-hidden border-2 border-transparent hover:border-indigo-600 transition-all shadow-sm group relative">
                      <img src={h.image} className="w-full aspect-[3/4] object-cover" />
                      <div className="absolute inset-0 bg-indigo-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><Maximize2 className="text-white w-6 h-6" /></div>
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden flex flex-col min-h-[820px]">
            ...
            <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Neural Stability: 99.8% • Biometric Anchor LOCKED</p>
                </div>
              </div>
                ...
                {resultImage && (
                  <>
                    <div className="flex bg-slate-100 p-1.5 rounded-2xl">
                      <button onClick={() => setViewMode('slider')} className={`p-2.5 rounded-xl transition-all ${viewMode === 'slider' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}><ArrowRightLeft className="w-5 h-5" /></button>
                      <button onClick={() => setViewMode('comparison')} className={`p-2.5 rounded-xl transition-all ${viewMode === 'comparison' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}><ImageIcon className="w-5 h-5" /></button>
                    </div>
                    <button onClick={downloadCollage} title="Download Comparison Collage" className="p-3.5 bg-slate-900 text-white rounded-2xl hover:bg-black transition-all active:scale-90"><Grid className="w-5 h-5" /></button>
                    <button onClick={() => { const a = document.createElement('a'); a.href = resultImage; a.download = 'style-render.png'; a.click(); }} className="p-3.5 bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-transform active:scale-90"><Download className="w-6 h-6" /></button>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 bg-[#fcfcfd] flex items-center justify-center p-10 relative overflow-hidden">
              ...
              {resultImage && (
                <div className="w-full h-full max-w-2xl rounded-[3rem] overflow-hidden shadow-[0_25px_60px_rgba(79,70,229,0.15)] bg-white p-4 border border-slate-100">
                  {viewMode === 'slider' ? (
                    <div ref={sliderRef} className="relative w-full h-full rounded-[2rem] overflow-hidden cursor-ew-resize select-none" onMouseMove={e => {
                      const rect = sliderRef.current.getBoundingClientRect();
                      setSliderPos(((e.clientX - rect.left) / rect.width) * 100);
                    }}>
                      <img src={resultImage} className="absolute inset-0 w-full h-full object-cover" />
                      <div className="absolute inset-0 w-full h-full" style={{ clipPath: `inset(0 ${100 - sliderPos}% 0 0)` }}>
                        <img src={userPhoto} className="absolute inset-0 w-full h-full object-cover" />
                      </div>
                      <div className="absolute top-0 bottom-0 w-1 bg-white shadow-2xl" style={{ left: `${sliderPos}%` }}>
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-2xl border-4 border-indigo-600"><ArrowRightLeft className="w-6 h-6 text-indigo-600" /></div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-6 h-full p-2">
                      <div className="relative rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-sm"><img src={userPhoto} className="w-full h-full object-cover" /></div>
                      <div className="relative rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-2xl"><img src={resultImage} className="w-full h-full object-cover" /></div>
                    </div>
                  )}
                </div>
              )}
            ...
            {error && <div className="p-4 bg-red-50 text-red-600 text-center text-[10px] font-black uppercase tracking-widest">{error}</div>}
          </div>
        </div>
      </main>
